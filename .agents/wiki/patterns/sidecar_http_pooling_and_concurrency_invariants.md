...
專案在處理 Real-Debrid API 呼叫、多連結 Unrestrict 解析、前端表格過濾計算、以及可執行檔打包發布時面臨以下效能與穩定性挑戰：
...
5. **連線池標頭遺失**：連線池復用時以 `client.session = session` 替換 session，但 `state.rd_session` 未預置 `Authorization` 標頭，造成帶 Token override 的單次測試成功（走獨立 session），但常規查詢（`rd_check_user` 走 shared session）因遺失驗證標頭回傳 HTTP 401（`rd_token_invalid`），UI 顯示「Token 無效或已過期」。
6. **發布門禁之機密掃描與進程鎖定**：測試碼中若使用隨機假字串如 `Bearer valid-token-123` 會觸發發布管線之源碼機密掃描（`test-release-scan.ps1` 失敗）；且若先前測試執行之 `javdbmagnet.exe` 未關閉，Windows 檔案鎖會使 `build-release.ps1` 在清理 staging 目錄時報 `Access denied`。
...
- **Session 屬性替換盲區**：`requests.Session` 之 `headers` 字典綁定於個體 instance，簡單將外部實例賦值給 `client.session` 會丟失原本建構式內寫入之 `Authorization` 標頭。
- **發布機密掃描 Fail-Closed**：`scripts/build-release.ps1` 對任何未經 allowlist 精確登記的 `Bearer <string>` 均視為憑證外洩拒絕發布。
- **Windows PE 執行期鎖定**：正處於執行狀態的二進位檔無法被 `Remove-Item` 覆寫或刪除。
...
- **測試夾具安全約束**：測試檔案中凡涉及 `Bearer` 標頭之模擬 token，必須採用 `scripts/release-scan-allowlist.txt` 既有登記之安全白名單標籤（如 `tok-xyz`），禁止發明新格式觸發發布門禁攔截。
- **建置前進程防護**：執行發布建置（`build-release.ps1`）前，必須先確保本機無正在執行的 `javdbmagnet.exe` 或 `sidecar.exe` 實例，避免 Windows 檔案鎖引發 Access Denied。