...
### 3.1 執行期命令硬性阻擋護欄
- 入口與空字串拒絕，位於 `.agent-hooks/recursive_delete_guard.py:214-217`：
  ```python
  ...
  ```
- 例外只豁免它自己那一段，剩下的文字照常比對，位於 `.agent-hooks/recursive_delete_guard.py:237-241`：
  ```python
  ...
  > 註冊，`test_exhaustive.py` 196/196 通過（2026-09-23 兩度修正例外豁免範圍後）。
